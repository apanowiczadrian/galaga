...
// ⚙️ KONFIGURACJA
const SHEET_NAME = 'postdata'; // Arkusz z danymi gry
const LOGIN_LOG_SHEET_NAME = 'loginlog'; // Arkusz z logami logowania

/**
 * Endpoint POST - odbiera dane z gry lub logi logowania
 */
function doPost(e) {
  try {
    // Parse JSON data
    ...
    // Check if this is a login log request
    if (data.action === 'loginlog') {
      return saveLoginLog(data);
    }

    // Otherwise, save game statistics (default behavior)
    return saveGameStats(data);
...
/**
 * Zapisz statystyki gry do arkusza 'postdata'
 */
function saveGameStats(data) {
  try {
    ...
    Logger.log('Error in saveGameStats: ' + error.toString());
    return ContentService.createTextOutput(JSON.stringify({
      success: false,
      error: error.toString()
    })).setMimeType(ContentService.MimeType.JSON);
...
/**
 * Zapisz log logowania do arkusza 'loginlog'
 */
function saveLoginLog(data) {
  try {
    // Otwórz spreadsheet i arkusz
    const sheet = SpreadsheetApp.getActiveSpreadsheet().getSheetByName(LOGIN_LOG_SHEET_NAME);

    if (!sheet) {
      return ContentService.createTextOutput(JSON.stringify({
        success: false,
        error: 'Sheet not found: ' + LOGIN_LOG_SHEET_NAME + '. Please run setupLoginLogSheet() first.'
      })).setMimeType(ContentService.MimeType.JSON);
    }
    ...
    const row = [
      ...
      data.nick || '',                                      // D: Nick
      data.email || '',                                     // E: Email
      data.browserName || 'Unknown',                        // F: Browser Name
      data.browserVersion || 'Unknown',                     // G: Browser Version
      data.isPWA ? 'Yes' : 'No',                           // H: isPWA
      data.fingerprint ? JSON.stringify(data.fingerprint) : '' // I: Browser Fingerprint (JSON)
    ];
    ...
    Logger.log('Error in saveLoginLog: ' + error.toString());
    return ContentService.createTextOutput(JSON.stringify({
      success: false,
      error: error.toString()
    })).setMimeType(ContentService.MimeType.JSON);
...
/**
 * Funkcja testowa dla loginlog - możesz uruchomić w Script Editor
 * 1. Wybierz "testLoginLog" z dropdown u góry
 * 2. Kliknij "Run"
 * 3. Sprawdź Google Sheet 'loginlog' - powinien pojawić się testowy wiersz
 */
...
/**
 * Setup funkcja - automatyczne tworzenie nagłówków dla arkusza 'postdata'
...
/**
 * Setup funkcja - automatyczne tworzenie nagłówków dla arkusza 'loginlog'
 * Uruchom raz na początku (wybierz z dropdown i kliknij Run)
 */
function setupLoginLogSheet() {
  const sheet = SpreadsheetApp.getActiveSpreadsheet().getSheetByName(LOGIN_LOG_SHEET_NAME);

  if (!sheet) {
    Logger.log('Error: Sheet "' + LOGIN_LOG_SHEET_NAME + '" not found!');
    Logger.log('Please create a sheet named "' + LOGIN_LOG_SHEET_NAME + '" first.');
    return;
  }
  ...
  Logger.log('✅ Login log sheet setup complete! Headers added and formatted.');
}